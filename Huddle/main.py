...
@login_required
def meeting():
    host_user_id = current_user.id  # Assuming current_user contains the host's user ID
    # Add logic to check if user is host and fetch participant list (if needed)
    return render_template("meeting.html", username=current_user.username, host_user_id=host_user_id)
